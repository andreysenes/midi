...
// Hipótese weltenschule — uma de quatro (Casio / KIrev / KOrev / ambos).
...
static void printHypotheses(uint8_t row, uint8_t col) {
  ...
  Serial.print(nameAt(row, kiRev));
  Serial.print(F("  KOrev:"));
  Serial.print(nameAt(koRev, col));
  Serial.print(F("  ambos:"));
  Serial.println(nameAt(koRev, kiRev));
}
...
static void printMap() {
  ...
  Serial.println(F("KI5-7 mudos = A#3 B3 C4 | F#4 G4 G#4 | D5 D#5 E5 | A#5 B5 C6"));
  Serial.println(F("ignorar: tempo+/- vol+/- rhythm (KI5-7). volume = EC11"));
}
...
static uint8_t learnIndexToCode(uint8_t i) {
  ...
  if (i < 42) {
    return static_cast<uint8_t>(200 + (i - 32));
  }
  if (i == 42) {
    return 215;
  }
...
static void learnStart() {
  ...
  Serial.println(F("32 teclas F3..C6, depois 0-9, stop, demo. Sem tempo/vol/rhythm."));
  printLearnPrompt();
}

static void learnRecord(const uint8_t *rows, const uint8_t *cols, uint8_t n) {
  learnedN[learnStep] = n;
  for (uint8_t k = 0; k < n && k < 4; k++) {
    learnedKo[learnStep][k] = static_cast<int8_t>(rows[k]);
    learnedKi[learnStep][k] = static_cast<int8_t>(cols[k]);
  }
...
static void learnBack() {
  ...
  if (learnStep == 0) {
    Serial.println(F("ja no passo 1/44"));
    printLearnPrompt();
    return;
  }
  learnOn = true;
  ...
  while (Serial.available()) {
    ...
    case 'l':
      ...
      if (learnOn) {
        ...
        Serial.print(learnStep + 1);
        Serial.println(F("/44. z=reiniciar | b=voltar | r=sair"));
        printLearnPrompt();
        break;
      }
      learnStart();
